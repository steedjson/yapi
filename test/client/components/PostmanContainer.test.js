...
//
// 规范化策略（比批 1 的 InterfaceColContentContainer 序列化器更严）：
//   - 保留：tag / class（剥离 antd CSS-in-JS 哈希）/ 全部 attributes / 文本（空白折叠）；
//   - style：批 1 整体剥离；本文件按「布局类白名单」采集 display / flex-basis /
//     flex-grow / visibility（值截断到白名单声明、按名排序），布局漂移可检出，
//     其余内联样式仍剥离；
//   - 受控值：批 1 只采集 attributes，受控输入的实时值（React 写在元素属性上、
//     不写进 attribute）不可见——本批修正为「对 input/textarea/select 采集实时
//     value / checked（checkbox·radio 取 checked，其余取 value）」，并剥离同名
//     attribute 避免重复；灵敏度用例②钉住该采集确实生效；
//   - 其余沿用：SVG path 的 d 数据、rc-select 与 react useId 生成 id、dnd-kit 自增 id
//     归一化（与结构无关且跨挂载不稳定），并新增 rc-motion 过渡态类名归一化
...
// 布局类白名单：批 1 整体剥离 style，此处保留这 4 个声明，布局漂移可检出
