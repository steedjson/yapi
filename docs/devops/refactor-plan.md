...
- 增加接口分类和菜单接口的兼容类型声明；
- 通过 `npm run typecheck` 后，再逐步将无行为变化的模块迁移为 TypeScript；当前已对 `server/utils/ttlCache.js` 和 `server/utils/categoryTree.js` 开启 `checkJs`，不改变运行时模块格式。分类树已从控制器中提取为独立工具，并保留历史平铺、缺失父节点和异常循环数据兼容。
