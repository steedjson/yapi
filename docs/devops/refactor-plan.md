...
- 通过 `npm run typecheck` 后，再逐步将无行为变化的模块迁移为 TypeScript；当前已对 `server/utils/ttlCache.js`、`server/utils/categoryTree.js`、`server/utils/token.js` 和 `server/utils/storage.js` 开启 `checkJs`，并补充了轻量 Node 运行时与加密/存储模块的 JSDoc/d.ts 类型声明，不改变运行时模块格式与 CommonJS 导出。
