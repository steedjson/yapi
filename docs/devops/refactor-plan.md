...
- 已修复接口列表「接口分类」列对子分类接口显示 catid 数字的问题（提交 `094edb49`）。根因：`get_cat_tree` 返回树（仅根节点），而分类列 `<Select>` 仅遍历顶层生成 `<Option>`，子分类缺失导致 `catid` 无匹配 `value` 而回退显示数字。修复在 `common/utils.js` 新增带环保护的纯函数 `flattenCatList`，分类列改为对拍平后的全量分类生成 `<Option>`（并修正 `Option` 的 React `key` 为 `cat._id`），不改任何后端 API 契约。配套回归测试 `test/common/flattenCatList.test.js`（7 用例），并以真实数据核对：项目 22 分类树顶层 48 个、拍平 80 个，585 个接口中 160 个位于子分类，拍平后全部 `catid` 均可匹配分类名称（0 未覆盖）。
- 2026 年 9 月 13 日，客户端 Redux 层全面纳入 TypeScript 严格类型检查（提交 `fb02e4e9`）。全部 10 个 reducer 模块（`menu.js`、`follow.js`、`mockCol.js`、`news.js`、`group.js`、`interface.js`、`interfaceCol.js`、`addInterface.js`、`project.js`、`user.js`）+ `create.js` 开启 `checkJs: strict`，JSDoc 类型注解覆盖全部函数参数与返回值，补充三方模块声明（axios/mockjs/json5/qs/redux/redux-promise/immer/react-router(-dom)/client plugin），运行时逻辑零改动。至此 Phase 14 的迁移顺序（类型检查配置 → server/utils → 分类服务和接口保存辅助逻辑 → 控制器 → Redux 和前端组件）已全面完成，服务端 + 客户端共 28 个核心模块均在 `tsc --noEmit strict` 下 0 错误。
...
- 导入弹窗（提交 `31f5dc55`）：ImportInterface.js（257 行）与 ProjectData.js（586 行）纳入严格检查，`global.d.ts` 追加 url 模块（parse/format 最小签名）、antd Table/Upload/Spin/Checkbox 与 HandleImportData 声明，零 `@ts-ignore`；babylon AST 全等对比证明无运行时语义变更，原有注释零丢失。验证同上（tsc 检查 227 文件、236 pass/0 fail、build 退出码 0）。

页面 URL、路由、表单字段、权限判断、接口调用、按钮含义和插件入口均未改动。**至此 Phase 15 五个页面（登录页→项目列表→接口分类树→接口编辑页→导入弹窗）全部完成。**
